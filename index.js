...
function generateNameBasedOnAdj () {
  const adjIndex = adj.length;
  let amountOfAdjIndex = Math.floor(Math.random() * adjIndex);
  let catchAdjData = adj[amountOfAdjIndex];
  return catchAdjData;
}

function generateNameBasedOnNoun () {
  const nounIndex = noun.length;
  let amountOfNounIndex = Math.floor(Math.random() * nounIndex);
